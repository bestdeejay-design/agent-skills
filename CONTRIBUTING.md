...
### Версионирование: версия скилла ≠ версия каталога

Это **два независимых трека** — не путайте их:

| Трек | Где живёт | Когда меняется |
|---|---|---|
| Версия скилла | `version` в `skills/<name>/skill.json` (semver) | при изменении самого скилла |
| Версия каталога | `version` на верхнем уровне `index.json` (semver) | при изменении состава/состояния коллекции |

Правила:
- Изменение одного скилла **не** поднимает автоматически версию каталога — версия каталога самостоятельна.
- Версия каталога меняется по состоянию коллекции: добавление/удаление скиллов или заметное обновление — minor; точечные правки — patch.
- При обновлении скилла: поднимите `version` и `updated` в его `skill.json` **и** синхронизируйте запись в `index.json` (version, triggers, description). Дрейф `index.json` ↔ `skill.json` запрещён.
- CI-валидатор (`validate_skills.py`) проверяет наличие обязательных полей, но **не сверяет версии между файлами** — синхронизация на вашей ответственности.
