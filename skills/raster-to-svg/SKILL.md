...
- `web/index.html` — фронтенд: drag & drop (PNG/JPG/WEBP с локальной перекодировкой), sticky-превью с лупой ×4 на результате, быстрые пресеты (например «Плоский стиль»), затемнение неактуальных настроек движка, прогресс-бар, код SVG, скачивание
...
- API: `GET /` — UI, `GET /health` — статус, `POST /convert?<параметры>` с телом = raw PNG → `{"svg", "report"}`.
...
|--------|-------------|----------------|
| `vtracer` (бинарь `vtracer` из пакета `vtracer-cli`) | `--engine vtracer` (cargo install vtracer-cli) | качественная цветная трассировка, лучше детали, в 10–30× быстрее на фото |
...
| фото 512×512 | 9.85 с, 490 KB, 8 путей | 0.44 с, 312 KB, 642 пути |
| фото 1024×1024 | 59.8 с, 1.2 MB, 8 путей | 2.0 с, 842 KB, 2014 путей |

Итог: для фото `vtracer` в 10–30× быстрее, легче и детальнее; `native` — выбор для простых изображений (логотипы, пиксель-арт, иконки): mce 0.0, один путь на цвет, полный офлайн. `auto` сам выбирает vtracer при наличии.
