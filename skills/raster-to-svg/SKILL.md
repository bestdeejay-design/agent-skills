...
- `web/` — фронтенд (`index.html` + `style.css` + `app.js`): drag & drop (PNG/JPG/WEBP с локальной перекодировкой), sticky-превью с лупой ×4 на результате, быстрые пресеты (например «Плоский стиль»), затемнение неактуальных настроек движка, прогресс-бар, код SVG, скачивание
