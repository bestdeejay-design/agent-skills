...
- Нужен PDF с графиками (встраивает Chart.js в HTML перед конвертацией)

Do NOT use when:
- Достаточно HTML — проще отдать `.md` или github-rendered страницу
- Нужна презентация — это `presentation-maker`
- Нужна интерактивная диаграмма — это `diagram-maker` / `data-analysis`
...
# Markdown с графиками → PDF:
# 1. Сгенерируй HTML с графиками через data-analysis --output html
# 2. Конвертируй HTML в PDF через pdf-report-builder
python3 skills/pdf-report-builder/scripts/pdf_report_builder.py -i report.html -o report.pdf --title "Отчёт с графиками"
```
...
Для PDF с графиками используй пайплайн:
1. `data-analysis --output html` — генерирует HTML с Chart.js графиками
2. `pdf-report-builder -i report.html -o report.pdf` — конвертирует в PDF

HTML-отчёт автоматически включает:
- Гистограммы для числовых полей
- Bar charts для категорий
- Scatter plots для корреляций
- Монохромную палитру (или porcelain/palm через `--preset`)
