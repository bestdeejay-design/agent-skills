...
- **Морфинг волн (d-path)**: анимация формы `animate attributeName="d"` с
  **4 кадрами** (`keyTimes="0;0.333;0.667;1"`, `calcMode="spline"`) и гребнями
  `Q`+`T` (smooth quadratic) — волна реально деформируется («текущая вода»),
  а не сдвигается. Во всех кадрах обязательна **одинаковая последовательность
  команд**; 4-й кадр = 1-й (замкнутый цикл). Приём из референса
  capsule-render `type=waving` (там `calcmod` — опечатка; у нас корректный
  `calcMode` + spline).
- **Блик-проход**: редкий (dur 16s) проход светлого блика от края до края —
  статичный градиент-ореол (пик stop-opacity 0.25, растушёванные хвосты
  0.10/0.20) + анимация `transform` самого `<rect>` (не `gradientTransform` —
  ненадёжно в Safari), opacity-окно синхронизировано с проходом, телепорт
  обратно при opacity=0. Первый проход сразу после загрузки.
- **Рассинхрон волн**: задержки 30% периода (`0s` / `-1.8s` / `-3.6s` при
  `dur="6s"`) — слои никогда не совпадают по фазе; пики `d`-path каждого слоя
  смещены относительно других (иначе слои сливаются).
