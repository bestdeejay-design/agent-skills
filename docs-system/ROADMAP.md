...
Shipped: SKILL.md + catalog/order/levels/completeness + 14 templates + pmOS reference.

Verified: templates are clean of generation artifacts; level decision table exists;
fill order is explicit; completeness gate exists.
...
| 2 | **Per-template "fill me" placeholders doc** | Templates are skeletons; a note describing how to fill each section consistently (single consistent voice). | New doc or section in `catalog.md` lists, per template, which sections are facts vs judgement. |
...
| 11 | **Multi-{open- close} catalog** | Support simple projects with only L1 without the noise of the full catalog. | `catalog.md` has explicit L1-only view: which docs, which sections of each, which to skip. |
